...
def recsys_add_user_by_ultragcn(new_user_preference : list) -> int:

    # 폴더 위치에 따라 수정 필요
    file_path = '/opt/ml/Recipe_Project/Recipe_code/ultragcn/data/Ultragcn_Recipe_Data/ultragcn_recipe_all_train_data_test_purpose.txt'
    LE_path = '/opt/ml/Recipe_Project/Recipe_code/ultragcn/label_encoder.pkl'
    recipe_LE = joblib.load(LE_path)

    new_user_preference = recipe_LE.transform(new_user_preference)

    try:
        # 기존 마지막 번호를 읽어옴
        with open(file_path, 'r') as file:
            last_number = int(file.readlines()[-1].split()[0])
    except FileNotFoundError:
        # 파일이 존재하지 않을 경우, 초기 번호인 1로 설정
        last_number = 0

    # 새로운 번호를 생성하고 파일에 추가
    new_number = last_number + 1
    with open(file_path, 'a') as file:
        file.write(f"{new_number} {' '.join(map(str, new_user_preference))}\n")
...
def recsys_get_recipe_by_ultragcn(new_user_num : int) -> list:
    LE_path = '/opt/ml/Recipe_Project/Recipe_code/ultragcn/label_encoder.pkl'
    recipe_LE = joblib.load(LE_path)
