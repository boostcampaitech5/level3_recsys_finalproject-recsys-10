...
@router.post("/predict/cosin", response_model=Response)
async def predict_by_cosin(ingre_list):
    result = await predict_cosin(ingre_list)
    return {
        "status_code": 200,
        "response_type": "success",
        "description": "Successfully predicted by jaccard similariy",
...
async def predict_cosin(ingre_list : str):
    input_list = ast.literal_eval(ingre_list)
    save_pickle_path = os.path.join(path, 'Ingredient_Recsys_pkl.pkl')
    return get_recipe_by_ingredient_based_recsys(input_list, save_pickle_path)